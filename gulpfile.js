 ...
 */
gulp.task('build', ['optimize', 'images', 'fonts', 'nginx'], function() {
...
gulp.task('build-only', ['optimize-build-only', 'images', 'fonts', 'nginx'], function() {
    log('Building everything');

    var msg = {
        title: 'gulp build-only',
        subtitle: 'Deployed to the build folder with no tests',
        message: 'Running `gulp build-only`'
    };
    del(config.temp);
    log(msg);
    notify(msg);
...
/**
 * Optimize all files without running any tests, move to a build folder,
 * and inject them into the new index.html
 * @return {Stream}
 */
gulp.task('optimize-build-only', ['inject'], function() {
    log('Optimizing the js, css, and html');
